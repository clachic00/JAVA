...
select substr (hiredate,1,2 )as YY,substr(hiredate,4,2 )as MON,substr(hiredate,7,2 )as DD from emp;
...
select count(count(mgr))from emp where mgr is not null group by mgr;
...
select min(sal) from emp where mgr is not null group by job having min(sal)>=2000 order by min(sal)desc;
...
select job,deptno as DNO, decode(deptno, 10, sal)as 부서10, decode(deptno, 20,sal)as 부서20,decode(deptno, 30,sal)as 부서30, sal as 총액 from emp order by deptno asc;

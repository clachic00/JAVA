...
select ename, mgr from emp where mgr=7788;
...
select deptno, ename from emp where deptno = 20 or deptno =30 order by ename desc;

--8. 사원의 급여가 2000에서 3000사이에 포함되고 부서번호가 20 또는 30인 사원의 이름, 급여와 부서번호를 출력, 이름순(오름차순)으로 출력하시오.

select ename, sal, deptno from emp where (sal>=2000 and sal<=3000) and deptno in (20,30) order by ename asc;
...
select ename, job, sal from emp where (job ='CLERK' or job = 'SALESMAN') and not sal in (1300,950,1600); 

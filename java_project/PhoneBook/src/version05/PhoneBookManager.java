...
import version05_Exception.StringEmptyException;
...
public class PhoneBookManager {
	...
	static private PhoneBookManager pbm = new PhoneBookManager();
	...
	void createInstance() {
		...
		while (true) {
		System.out.println(
				" " + MenuInterface.UNIV + ".대학 " + MenuInterface.COMPANY + ".회사 " + MenuInterface.CAFE + ".동호회");
		System.out.println("===================================");
	
		try {
			select = Integer.parseInt(sc.nextLine());
			// select = sc.nextInt();
			if (!(select >= MenuInterface.UNIV && select <= MenuInterface.EXIT)) {
